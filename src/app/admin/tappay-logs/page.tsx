...
const ACTION_LABELS: Record<string, string> = {
  payByPrime: '首次付款（Pay by Prime）',
  payByToken: '已存卡付款（Pay by Token）',
  notify: '付款結果通知（webhook）',
}
const STATIC_ACTIONS = ['payByPrime', 'payByToken', 'notify']

export default function TapPayLogsPage() {
  const [logs, setLogs] = useState<TapPayLog[]>([])
  ...
  async function load() {
    ...
    const res = await fetch(`/api/admin/tappay-logs?${params}`)
    if (res.ok) {
  ...
  async function toggleExpand(id: string) {
    ...
    if (!wasExpanded && !bodyCache.has(id)) {
      const res = await fetch(`/api/admin/tappay-logs/${id}`)
      if (res.ok) {
        const d = await res.json()
  ...
  return (
    <div>
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold">TapPay API Log</h1>
          <p className="mt-1 text-sm text-gray-500">共 {total} 筆記錄 · 付款/已存卡付款/webhook 全留底（敏感欄位已遮罩）</p>
        </div>
        ...
        {filterAction && (
          <button
            onClick={async () => {
              ...
              const res = await fetch(`/api/admin/tappay-logs?action=${filterAction}`, { method: 'DELETE' })
              const d = await res.json()
      ...
      {loading ? <p className="mt-8 text-sm text-gray-500">載入中...</p> : logs.length === 0 ? (
        <div className="mt-8 text-center py-16 bg-white rounded-xl border border-gray-200">
          <p className="text-gray-500">尚無 TapPay API 記錄</p>
        </div>
      ) : (
            ...
            return (
              <div key={log.id} className="bg-white border border-gray-200 rounded-lg overflow-hidden">
                <div className="flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50" onClick={() => toggleExpand(log.id)}>
                  ...
                  {log.tappay_status != null && <span className="text-[10px] text-gray-400 flex-shrink-0">status={log.tappay_status}</span>}
                ...
                {expanded && (
                  <div className="border-t border-gray-100 px-4 py-3 bg-gray-50">
                    <div className="mb-2 text-[11px] text-gray-500 font-mono">{log.endpoint} {log.trade_id ? `· rec_trade_id=${log.trade_id}` : ''}</div>
                    {(() => {

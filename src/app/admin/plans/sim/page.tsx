...
interface BCProduct {
  ...
  prices: PriceItem[] | null; country_data: CountryItem[] | null; is_active: boolean
}

export default function AdminSimPlansPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/plans?type=sim').then((r) => r.ok ? r.json() : []).then(setProducts).finally(() => setLoading(false))
  }, [])
  ...
  const daysOptions = useMemo(() => {
    ...
    products.forEach((p) => { const u = p.days ?? 1; if (p.prices?.length) p.prices.forEach((t) => set.add(u * parseInt(t.copies))); else set.add(u) })
    return Array.from(set).sort((a, b) => a - b)
  }, [products])

  const capacityOptions = useMemo(() => {
    const map = new Map<string, boolean>()
    products.forEach((p) => { const k = p.high_flow_size ?? p.capacity; if (k) map.set(k, p.plan_type === '1') })
    return Array.from(map.entries()).sort(([a], [b]) => parseFloat(a) - parseFloat(b)).map(([v, d]) => ({ value: v, label: formatCapacity(v, d) }))
  }, [products])

  const filtered = useMemo(() => products.filter((p) => {
    if (search) { const q = search.toLowerCase(); if (!p.name.toLowerCase().includes(q) && !p.sku_id.toLowerCase().includes(q)) return false }
    if (filterDays) { const t = parseInt(filterDays), u = p.days ?? 1; if (!p.prices?.some((pr) => u * parseInt(pr.copies) === t)) return false }
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold">SIM 套餐</h1>
      <p className="mt-1 text-sm text-gray-500">共 {filtered.length} 個套餐</p>

      <div className="mt-4 flex flex-wrap gap-3">
        <select value={filterDays} onChange={(e) => setFilterDays(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
          <option value="">天數</option>
          {daysOptions.map((d) => <option key={d} value={d}>{d} 天</option>)}
        </select>
        <select value={filterCapacity} onChange={(e) => setFilterCapacity(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm">
      ...
      {loading ? <p className="mt-8 text-sm text-gray-500">載入中...</p> : filtered.length === 0 ? (
        <p className="mt-8 text-gray-500 text-center">尚無 SIM 卡商品</p>
      ) : (
                  ...
                  return (
                    <Fragment key={product.id}>
                      <tr className={`hover:bg-gray-50 ${hasPrices ? 'cursor-pointer' : ''} ${isExpanded ? 'bg-blue-50/50' : ''}`} onClick={() => hasPrices && toggleExpand(product.id)}>
                        ...
                        <td className="px-4 py-3"><span className="px-1.5 py-0.5 text-xs rounded bg-green-50 text-green-600 font-medium">{product.type}</span><div className="text-xs text-gray-400 mt-0.5">{isDaily ? '單日型' : '總量型'}</div></td>

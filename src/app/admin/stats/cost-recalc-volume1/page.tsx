...
export default function CostRecalcVolume1Page() {
  ...
  function exportCsv() {
    ...
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = 'cost-recalc-volume1.csv'; a.click(); URL.revokeObjectURL(url)
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold">成本重算（依一日流量價）</h1>
          <p className="mt-1 text-sm text-gray-500">用 1GB 基礎方案「1 天檔結算價 ÷ 1GB」當每GB價（即單日價），家族內所有品項一律「每GB價 × 實際總用量」計價；逐原始商品(SKU)呈現（基礎以下不動；¥ CNY）</p>
        </div>

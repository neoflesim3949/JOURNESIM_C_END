...
export default function CostRecalcPage() {
  ...
  async function syncAccel() {
    ...
    try {
      ...
      else { setSyncMsg(`同步完成：F056 共 ${j.total?.toLocaleString?.() || j.total} 筆、有報價 ${j.priced?.toLocaleString?.() || j.priced} 筆、寫入 ${j.upserted?.toLocaleString?.() || j.upserted} 筆`); load() }
  ...
  return (
    <div>
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h1 className="text-2xl font-bold">成本重算（依方案）</h1>
          <p className="mt-1 text-sm text-gray-500">把「每日型且 ≥1GB」的方案改買同區 1GB 基礎方案＋超量補 1GB 加速包，模擬採購成本能降多少（幣別 ¥ CNY）</p>
        </div>

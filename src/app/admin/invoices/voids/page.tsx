...
export default function VoidsPage() {
  ...
  async function load() {
    setLoading(true)
    const p = new URLSearchParams({ page: String(page), pageSize: '50', status: 'voided' })
  ...
  return (
    <div>
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold">註銷列表</h1>
          <p className="mt-1 text-sm text-gray-500">共 {total} 筆註銷發票（若為字軌最後一號則歸還）</p>
        </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-xs text-gray-600">
            <tr>
              ...
              <th className="px-3 py-2 text-left">註銷時間</th>
              <th className="px-3 py-2 text-left">註銷原因</th>
              <th className="px-3 py-2 text-center">明細</th>
            </tr>
          </thead>
          <tbody>
            {loading && <tr><td colSpan={8} className="px-3 py-6 text-center text-gray-400">載入中…</td></tr>}
            {!loading && rows.length === 0 && <tr><td colSpan={8} className="px-3 py-6 text-center text-gray-400">沒有註銷發票</td></tr>}
            {!loading && rows.map(r => (
              <tr key={r.id} className="border-t hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-xs text-gray-500">{r.voided_at ? new Date(r.voided_at).toLocaleString('zh-TW') : '—'}</td>
                <td className="px-3 py-2 text-xs">{r.void_reason || '—'}</td>

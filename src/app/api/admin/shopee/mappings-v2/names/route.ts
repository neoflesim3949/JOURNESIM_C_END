...
export async function PUT(request: Request) {
  ...
  if (type === 'main_sku') {
    ...
    const { error } = await supabase.from('shopee_product_options_v2')
      .update({ main_sku_code: val }).eq('account_id', account_id).eq('shopee_product_id', String(key))
  ...
  const { error } = await supabase.from('shopee_product_options_v2')
    .update(type === 'product' ? { custom_product_name: val } : { custom_variation_name: val })

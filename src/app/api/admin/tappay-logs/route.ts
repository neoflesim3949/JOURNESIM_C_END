...
// GET — TapPay API Log 列表（不含 JSONB body，展開單筆才另拉）
export async function GET(request: Request) {
  ...
  let query = supabase.from('tappay_api_logs')
    .select('id, action, endpoint, direction, order_number, trade_id, status, tappay_status, error_message, duration_ms, created_at', { count: 'estimated' })
...
export async function DELETE(request: Request) {
  ...
  const supabase = createAdminClient()
  const { error, count } = await supabase.from('tappay_api_logs').delete({ count: 'exact' }).eq('action', action)

...
// mode='cost'：比 BC 成本（結算/零售，¥）；mode='sell'：比套餐售價（NT$）
export default function PlanCompareModal({ plans: initial, onClose, mode = 'cost' }: { plans: ComparePlan[]; onClose: () => void; mode?: 'cost' | 'sell' }) {
  const [plans, setPlans] = useState<ComparePlan[]>(initial)
  const [useRetail, setUseRetail] = useState(false)
  const isSell = mode === 'sell'
  const currency = isSell ? 'NT$' : '¥'
  const priceLabel = isSell ? '售價' : (useRetail ? '零售價' : '結算價')
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-5xl max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-bold">{isSell ? '售價比較' : '成本比較'}</h2>
            <p className="text-xs text-gray-500 mt-0.5">依「相同 copies」對比{priceLabel}（每列最低價綠色標示）</p>
          </div>
          ...
          <>
            ...
            <div className="cmp-bar overflow-y-auto overflow-x-hidden flex-1 min-h-0">
              <div ref={bodyRef} onScroll={() => syncFrom('body')} className="cmp-noscroll overflow-x-auto">
                <PlanCompareTable plans={plans} useRetail={useRetail} currency={currency} showMargin={isSell} onRemove={(sku) => setPlans(prev => prev.filter(p => p.sku_id !== sku))} />
              </div>
            </div>
          </>
        )}
        <div className="px-5 py-2.5 border-t border-gray-100 text-xs text-gray-400">綠色＝該 copies 下最低{priceLabel}；「—」表示該套餐無此 copies；每日型另標對應天數。</div>
      </div>
    </div>
  )
}
